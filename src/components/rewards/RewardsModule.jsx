...
import { CARDS, getCard, bestCardForCategory } from '../../data/cards'
...
export default function RewardsModule({ transactions }) {
  const { earnedTotal, optimalTotal, perCard } = useMemo(() => {
    ...
    transactions.forEach((t) => {
      ...
      if (card) {
        const earned = t.amount * (card.rates[t.category] / 100)
        earnedTotal += earned
        perCard[card.id] += earned
      }
      const best = bestCardForCategory(t.category)
      optimalTotal += t.amount * (best.rates[t.category] / 100)
    })
